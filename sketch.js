...
function setup(){
  ...
  gameOverSpr = createSprite(300,200,10,10);
  gameOverSpr.addImage(gameOverSprImg);
  
 restartSpr = createSprite(300,300,10,10);
...
function fruits(){
  if(frameCount % 100 === 0){
  fruit = createSprite(Math.round(random(100,500)),600,10,10)
  fruit.velocityY = random(-9,-6);
    fruit.scale = 0.2;
    fruit.setCollider("rectangle",0,0,10,10);
    fruits_group.add(fruit);
    fruits_group.setLifetimeEach = 100;
...
function bombs(){
  if(frameCount % 60 === 0){
  bomb = createSprite(Math.round(random(100,500)),600,10,10)
  bomb.velocityY = random(-9,-6);
    bomb.scale = 0.6;
    bomb.setCollider("rectangle",0,0,20,20);
    bombs_group.add(bomb);
    bombs_group.setLifetimeEach = 100;
